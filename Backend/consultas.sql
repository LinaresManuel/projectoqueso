...
FROM ma_ventas v
JOIN ma_clientes c ON v.cliente_id = c.id
JOIN ma_lotes l ON v.lote_id = l.id;
...
FROM ma_pagos_clientes p
JOIN ma_clientes c ON p.cliente_id = c.id;
...
FROM ma_lotes l
JOIN ma_productos p ON l.producto_id = p.id;
...
FROM ma_recepciones r
JOIN ma_proveedores pr ON r.proveedor_id = pr.id;
...
FROM ma_ingresos_productos i
JOIN ma_productos pr ON i.producto_id = pr.id
...
-- Eliminar cliente
DELETE FROM ma_clientes
WHERE id = ?;

-- Eliminar una venta específica
DELETE FROM ma_ventas WHERE id = ?;